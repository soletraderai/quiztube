...
import type { Lesson, Library, ProcessingState, CodeSnippet, EvaluationResult, LessonProgress } from '../types';
...
const sessionApi = {
  ...
  async fetchSessions(): Promise<Lesson[]> {
  ...
  async saveSession(session: Lesson): Promise<void> {
    const headers = getAuthHeaders();
  ...
  async updateSession(session: Lesson): Promise<void> {
    const headers = getAuthHeaders();
    if (!headers) return;
  ...
  async completeSession(session: Lesson): Promise<void> {
    const headers = getAuthHeaders();
    if (!headers) return;
  ...
  async logCommitment(session: Lesson): Promise<void> {
    const headers = getAuthHeaders();
    if (!headers) return;
...
interface LessonState {
  ...
  // Lesson management
  createSession: (session: Lesson) => void;
  updateSession: (sessionId: string, updates: Partial<Lesson>) => void;
  deleteSession: (sessionId: string) => void;
  getSession: (sessionId: string) => Lesson | undefined;

  // Current session
  setCurrentSession: (session: Lesson | null) => void;

  // Processing state
  setProcessingState: (state: ProcessingState | null) => void;

  // Topic/Question updates
  updateTopic: (sessionId: string, topicIndex: number, updates: Partial<Lesson['topics'][0]>) => void;
  updateQuestion: (sessionId: string, topicIndex: number, questionIndex: number, updates: Partial<Lesson['topics'][0]['questions'][0]>) => void;

  // Score updates
  updateScore: (sessionId: string, updates: Partial<Lesson['score']>) => void;
  ...
  updateSessionProgress: (sessionId: string, progress: Partial<LessonProgress>) => void;
  ...
  persist(
    (set, get) => ({
      ...
      updateEvaluationScore: (sessionId, result) =>
        set((state) => {
          const getUpdates = (score: Lesson['score']) => {
            const updates: Partial<Lesson['score']> = {};
            switch (result) {
              case 'pass':
...
export const useSessionStore = useLessonStore;
